...
readonly POLKADOT_SNAP_NAME="${POLKADOT_SNAP_NAME:-polkadot-parachain}"
readonly POLKADOT_SNAP_SERVICE="snap.${POLKADOT_SNAP_NAME}.${POLKADOT_SNAP_NAME}.service"
readonly POLKADOT_SERVICE_INSTANCE="${POLKADOT_SNAP_NAME}.${POLKADOT_SNAP_NAME}"
readonly POLKADOT_DISPLAY_NAME="${POLKADOT_DISPLAY_NAME:-Polkadot Parachain}"
...
check_polkadot_service_running() {
    echo "Checking if ${POLKADOT_DISPLAY_NAME} service is running..."

    if snap services "${POLKADOT_SNAP_NAME}" | awk -v service="${POLKADOT_SERVICE_INSTANCE}" '$1 == service && $3 == "active" { found=1 } END { exit(found ? 0 : 1) }'; then
        echo "${POLKADOT_DISPLAY_NAME} service is running."
        return 0
    fi

    echo "${POLKADOT_DISPLAY_NAME} service is not running."
    return 1
}
...
wait_for_polkadot_service() {
    ...
    echo "Waiting for ${POLKADOT_DISPLAY_NAME} RPC to start on port 9933..."

    while (( elapsed < timeout )); do
        if nc -z localhost 9933 >/dev/null 2>&1; then
            echo "${POLKADOT_DISPLAY_NAME} RPC is listening on port 9933."
            return 0
        fi
    ...
    echo "Timed out waiting for the ${POLKADOT_DISPLAY_NAME} RPC port." >&2
    return 1
}
...
resolve_chain_spec_filename() {
    ...
    case "${chain_name}" in
        polkadot-assethub)
            echo "asset-hub-polkadot.json"
            ;;
        kusama-assethub)
            echo "asset-hub-kusama.json"
            ;;
        bridge-hub-polkadot)
            echo "bridge-hub-polkadot.json"
            ;;
        people-polkadot)
            echo "people-polkadot.json"
            ;;
        *)
            echo "Unsupported test chain '${chain_name}'." >&2
            return 1
            ;;
    esac
}
...
get_chain_argument() {
    ...
    stage_chain_spec_file "${POLKADOT_TEST_CHAIN:-polkadot-assethub}"
}
...
wait_for_node_health() {
    ...
    echo "Waiting for ${POLKADOT_DISPLAY_NAME} to get peers and start syncing..."
...
PY
        then
            echo "${POLKADOT_DISPLAY_NAME} has peers and is syncing."
            return 0
        fi
    ...
    echo "Timed out waiting for ${POLKADOT_DISPLAY_NAME} to get peers and start syncing." >&2
    return 1
}
