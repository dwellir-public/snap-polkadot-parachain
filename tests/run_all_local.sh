...
cleanup() {
    ...
    echo
    echo "==== Cleanup ===="
    sudo snap remove polkadot-parachain --purge >/dev/null 2>&1 || true
...
run_runtime_test "Asset Hub Polkadot initial install" polkadot-assethub tests/test_initial_install.sh
run_step "Base-path validation" bash tests/test_base_path.sh
run_revision_test "Asset Hub Polkadot downgrade" tests/test_downgrade.sh
run_runtime_test "Asset Hub Polkadot initial install before endure" polkadot-assethub tests/test_initial_install.sh
run_revision_test "Asset Hub Polkadot endure" tests/test_endure.sh

for chain in kusama-assethub bridge-hub-polkadot; do
